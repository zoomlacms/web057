                ...
                if (string.IsNullOrEmpty(PayMethod))
                {
                    ...
                    switch ((M_PayPlat.Plat)payPlat.PayClass)//现仅开通 12:支付宝即时到账和支付宝网银服务,15支付宝网银服务(上方代码中处理),银币与余额服务
                    {
                        ...
                        case M_PayPlat.Plat.Alipay_Instant:
                            #region 支付宝[即时到帐]
							//公益支付接口单独处理
							Response.Redirect("/api/pay/alipay_h5.aspx?PayNo=" + pinfo.PayNo);
							//普通即时到帐接口处理
        ...
        private void PayByVirtualMoney(string payMethod, M_Payment payMod)
        {
            ...
            for (int i = 0; i < orderList.Count; i++)//更改订单状态
            {
                ...
                if (userappMod != null)
                {
                    ...
                    string json = "{\"first\": {\"value\":\"订单付款通知\",\"color\":\"#173177\"},\"keyword1\":{\"value\":\"" + orderMod.OrderNo + "\",\"color\":\"#173177\"},\"keyword2\":{\"value\":\"" + DateTime.Now.ToString("yyyy年MM月dd日 HH:mm:ss") + "\",\"color\":\"#173177\"},\"keyword3\": {\"value\":\"" + orderMod.Ordersamount.ToString("0.00") + "元\",\"color\":\"#173177\"},\"remark\":{\"value\":\"您的订单已付款！\",\"color\":\"#173177\"}}";
                    wxapi.Tlp_SendTlpMsg(userappMod.OpenID, "7Vuvl7s1Oe5BNGmebCexo6PBJ7dnBnkYReqnYXMS5EY", "http://www.hjd7979.com/Class_14/NodeElite.aspx?OrderNo=" + orderMod.OrderNo, json);
                    //发送消息给上级
                    M_UserInfo pmu = buser.SeachByID(DataConvert.CLng(mu.ParentUserID));
                    M_UserAPP puserappMod = userappBll.SelModelByUid(pmu.UserID, "wechat");
                    if (puserappMod != null)
                    {
                        string json1 = "{\"first\": {\"value\":\"下级会员：[" + mu.UserName + "]订单付款通知\",\"color\":\"#173177\"},\"keyword1\":{\"value\":\"" + orderMod.OrderNo + "\",\"color\":\"#173177\"},\"keyword2\":{\"value\":\"" + DateTime.Now.ToString("yyyy年MM月dd日 HH:mm:ss") + "\",\"color\":\"#173177\"},\"keyword3\": {\"value\":\"" + orderMod.Ordersamount.ToString("0.00") + "元\",\"color\":\"#173177\"},\"remark\":{\"value\":\"" + mu.UserName + "的订单已付款！\",\"color\":\"#173177\"}}";
                        wxapi.Tlp_SendTlpMsg(puserappMod.OpenID, "7Vuvl7s1Oe5BNGmebCexo6PBJ7dnBnkYReqnYXMS5EY", "http://www.hjd7979.com/Class_14/NodeElite.aspx?OrderNo=" + orderMod.OrderNo, json1);
                    }
                }
            }
            //-----------------For End
